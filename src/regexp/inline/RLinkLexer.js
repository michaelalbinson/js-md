...
class RLinkLexer extends ARLexer {
    ...
    static getMarkup(text, link) {
        ...
        if (link.match(/(?:mailto:)?([-.\w]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)/))
            link = this.scrambleEmail(link);

        // if the text is exactly an email address, scramble it too
        if (text.match(/^(?:mailto:)?([-.\w]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)$/))
            text = this.scrambleEmail(text);
